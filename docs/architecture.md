...
js/cloud-auth.js           account login/register over an illustrated hero welcome screen (renderStart adds .auth-photo → game/assets/welcome-hero.png as the backdrop; a compact frosted login panel overlays it) with a classic show/hide password eye (authToggleEye); full-screen ADMIN dashboard (authOpenAdmin): approval waiting-list + per-player details (settings/level/arenas/per-arena stats); pushes progress to D1 every 25s (authStartProgressSync, admin/test account excluded)
