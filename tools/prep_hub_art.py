...
# Wonderland's 24 carnival booths (2026-08-05). Same cut-out treatment as the Earth Hub sprites, but
# they land in their own folder and, crucially, are OPTIONAL: the booths render as CSS-drawn tents
# until their picture exists (see .wmap-map-wonder in map.css). So instead of shipping 24 CSS rules
# that would 404 for every booth not yet drawn, this script writes WONDER_CSS containing a rule only
# for each image it actually produced. Adding art is then genuinely one command.
WONDER_OUT = r"C:\PythonProject\AlgebraGame\game\assets\wonder"
WONDER_CSS = r"C:\PythonProject\AlgebraGame\game\css\wonder-art.css"

# (source filename in picture/, booth id as used by data-id in the DOM)
WONDER_BOOTHS = [
    ("w-hoohey.png", "hoohey"),       ("w-slots.png", "slots"),
    ("w-poptic.png", "poptic"),       ("w-fishin.png", "fishin"),
    ("w-tileball.png", "tileball"),   ("w-forge.png", "forge"),
    ("w-match.png", "match"),         ("w-sudoku.png", "sudoku"),
    ("w-cargo.png", "cargo"),         ("w-glacier.png", "glacier"),
    ("w-forbidden.png", "forbidden"), ("w-stacker.png", "stacker"),
    ("w-astro.png", "astro"),         ("w-virus.png", "virus"),
    ("w-circuit.png", "circuit"),     ("w-comet.png", "comet"),
    ("w-blastbot.png", "blastbot"),   ("w-bubble.png", "bubble"),
    ("w-bowling.png", "bowling"),     ("w-rhythm.png", "rhythm"),
    ("w-snake.png", "snake"),         ("w-crystal.png", "crystal"),
    ("w-cloudberry.png", "cloudberry"), ("w-skysquad.png", "skysquad"),
]
...
def main():
    ...
    report.extend(_prep_wonderland())
    print("\n".join(report))


def _prep_wonderland():
    """Cut out whatever booth art exists and regenerate the CSS that points at it.

    Rewritten from scratch every run (not appended to) so deleting a source picture also removes its
    rule — otherwise a stale rule would 404 forever and the booth would silently lose its tent.
    """
    os.makedirs(WONDER_OUT, exist_ok=True)
    report, rules, missing = [], [], []

    for src, booth_id in WONDER_BOOTHS:
        p = os.path.join(SRC, src)
        if not os.path.exists(p):
            missing.append(booth_id)
            continue
        cut = trim_and_fit(remove_bg(Image.open(p)), SPRITE_PX)
        cut.save(os.path.join(WONDER_OUT, src), optimize=True)
        op = (np.asarray(cut)[:, :, 3] > 0).mean() * 100
        report.append(f"booth    {src:18s} {SPRITE_PX}x{SPRITE_PX}  opaque={op:5.1f}%  "
                      f"{os.path.getsize(os.path.join(WONDER_OUT, src))//1024}KB")
        rules.append(
            f'.wmap-map-wonder .wmap-building[data-id="{booth_id}"]{{\n'
            f"  background-image: url('../assets/wonder/{src}');\n"
            f"}}"
        )

    header = (
        "/* Wonderland booth artwork — GENERATED by tools/prep_hub_art.py. Do not hand-edit.\n"
        "   One rule per booth whose picture has actually been produced, so a booth without art yet\n"
        "   costs zero failed requests; it simply keeps the CSS-drawn tent from map.css\n"
        "   (.wmap-map-wonder). To add art: drop the PNGs named in\n"
        "   picture/WONDERLAND_BOOTH_PROMPTS.md into picture/, then run this script. */\n"
    )
    body = "\n".join(rules)
    if rules:
        # The art sits INSIDE the tent frame, so it needs the same footprint the tent uses.
        body = (".wmap-map-wonder .wmap-building[data-id]{\n"
                "  background-position: center bottom;\n"
                "  background-size: contain;\n"
                "  background-repeat: no-repeat;\n"
                "}\n") + body
    with io_open(WONDER_CSS, "w") as f:
        f.write(header + ("\n" + body + "\n" if body else ""))

    report.append(f"wonder   {len(rules)}/{len(WONDER_BOOTHS)} booths have art; "
                  f"still CSS tents: {', '.join(missing) if missing else 'none'}")
    report.append(f"wrote    {WONDER_CSS}")
    return report
